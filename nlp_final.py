...
class PromptResponseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_ids = torch.stack([inputs_a["input_ids"], inputs_b["input_ids"]], dim=0)
        attention_mask = torch.stack([inputs_a["attention_mask"], inputs_b["attention_mask"]], dim=0)
...
class BertForSequenceClassification(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        input_ids_a, input_ids_b = input_ids[:, 0], input_ids[:, 1]
        attention_mask_a, attention_mask_b = attention_mask[:, 0], attention_mask[:, 1]
...
def dpo_loss(outputs, labels, beta=1.0):
    # Compute difference and exponential
    logits_a, logits_b = outputs[:, 0], outputs[:, 1]
...
scheduler = scheduler = get_scheduler("linear", optimizer, num_warmup_steps=0, num_training_steps=len(train_dataloader) * num_epochs)
...
with torch.no_grad():
    ...
    for batch in loop:
        inputs_a = {k: v[:, 0].to(device) for k, v in batch.items()}
        inputs_b = {k: v[:, 1].to(device) for k, v in batch.items()}

        logits_a, logits_b = model(**inputs_a, **inputs_b)
        probs.extend(torch.sigmoid(torch.stack([logits_a, logits_b], dim=-1)).tolist())

        preds = torch.where(logits_a > logits_b, 0, 1)
